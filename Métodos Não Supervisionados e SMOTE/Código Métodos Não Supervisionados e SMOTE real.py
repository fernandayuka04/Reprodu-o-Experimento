...
# --- 3. Função para SMOTE (CORRIGIDA) ---

def apply_smote_to_normal_data(X_normal_scaled):
    """
    Aplica o SMOTE real para gerar amostras sintéticas da CLASSE NORMAL.

    CORREÇÃO: Inclui um ponto dummy de Classe 1 para satisfazer a validação do SMOTE,
    que exige mais de uma classe, e depois filtra o resultado.
    """
    original_count = X_normal_scaled.shape[0]

    # 1. Cria o ponto dummy de CLASSE 1 (e a classe 0 com seus rótulos dummy)
    X_dummy_point = X_normal_scaled[0:1]  # Pega a primeira amostra
    y_dummy_point = np.array([1], dtype=int)  # Rótulo falso de classe 1

    X_temp = np.vstack([X_normal_scaled, X_dummy_point])  # Combina X normal com o ponto dummy
    y_temp = np.hstack([np.zeros(original_count, dtype=int), y_dummy_point])  # Combina y dummy com o rótulo falso

    # 2. Define a estratégia de amostragem: Aumenta a Classe 0 para o dobro, mantém a Classe 1 em 1.
    sampling_strategy = {0: original_count * 2, 1: 1}

    print(f"  -> Aplicando SMOTE: {original_count} amostras -> {original_count * 2} amostras.")

    # 3. Instancia e aplica o SMOTE
    # O Counter(y_temp) agora retorna {0: original_count, 1: 1}, satisfazendo o imblearn.
    smote = SMOTE(sampling_strategy=sampling_strategy, k_neighbors=5, random_state=SEED)
    X_smote_resampled_temp, y_smote_resampled_temp = smote.fit_resample(X_temp, y_temp)

    # 4. Filtra o resultado: Retém SOMENTE as amostras da classe 0 (normal, incluindo as sintéticas)
    X_smote_resampled = X_smote_resampled_temp[y_smote_resampled_temp == 0]
...
def evaluate_anomaly_model(model, X_test, y_test, threshold, is_ae=False):
    ...
    if is_ae:
        ...
        # Para OCSVM e iForest, 1 é inlier (Normal), -1 é outlier (Fraude)
        y_pred_sklearn = model.predict(X_test)
        # Converte: -1 (Fraude) -> 1, 1 (Normal) -> 0
...
# 5.1.2 OCSVM com 'SMOTE' Sampling (Implementação Real)
...
print_results("OCSVM (SMOTE Sampling - Real)", results_ocsvm_smote)

# Parâmetros de Isolation Forest
IFOREST_PARAMS = {
    'n_estimators': 100,
    'max_samples': 'auto',
    'contamination': 0.1,
...
# 5.2.2 iForest com 'SMOTE' Sampling (Implementação Real)
...
print_results("iForest (SMOTE Sampling - Real)", results_iforest_smote)
...
# 5.3.2 AE com 'SMOTE' Sampling (Implementação Real)
...
print_results("AE (SMOTE Sampling - Real)", results_ae_smote)